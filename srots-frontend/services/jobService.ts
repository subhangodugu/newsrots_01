...
export const JobService = {
    ...
    // ── Eligible Students (used by JobEligibleTab) ────────────────────────────

    searchEligibleStudents: async (jobId: string, filters: { query?: string; branch?: string; status?: string }): Promise<Student[]> => {
        const response = await api.get(`/jobs/${jobId}/eligible-students`, { params: filters });
        return response.data || [];
    },

    exportJobEligibleStudents: async (jobId: string, format: string = 'excel'): Promise<void> => {
        const response = await api.get(`/jobs/${jobId}/export-list`, {
            params: { type: 'eligible', format },
            responseType: 'blob',
        });
