...
export const FreeCoursesSection: React.FC<FreeCoursesSectionProps> = ({ user }) => {
  ...
  const handleSave = async () => {
    ...
    const normalizedForm: Partial<FreeCourse> = {
      ...courseForm,
      platform: (courseForm.platform ? String(courseForm.platform).toUpperCase() : CoursePlatform.OTHER) as CoursePlatform,
    ...
    <div className="space-y-6 animate-in fade-in">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
              {technologies.map((tech) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap border transition-all ${techFilter === tech
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
              {platforms.map((p) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap border transition-all ${platformFilter === p
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative min-h-[400px]">
        ...
        {courses.map((course) => (
          <div
            key={course.id}
            className={`bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all relative group overflow-hidden flex flex-col h-full ${course.status === CourseStatus.INACTIVE ? 'opacity-75 bg-gray-50/50 border-dashed border-2' : ''
              }`}
          >
            {isSrotsUser && (
              <div className="absolute top-3 right-3 flex gap-1 z-10">
                <button
                  ...
                  className={`p-1.5 rounded-lg border bg-white shadow-sm transition-colors ${course.status === CourseStatus.ACTIVE
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg border bg-white shadow-sm transition-colors ${verifyingId === course.id ? 'animate-pulse text-blue-400' : 'text-blue-600 border-blue-100 hover:bg-blue-50'
                    }`}
                  title="Verify Link"
                >
                  <ShieldCheck size={14} />
      ...
      >
        <div className="flex-1 overflow-y-auto max-h-[70vh] custom-scrollbar p-6 space-y-5">
          ...
          <div className="bg-blue-50/50 p-4 rounded-xl flex items-center justify-between border border-blue-100">
            ...
            <select
              className={`p-1.5 rounded-lg text-[10px] font-bold border outline-none ${courseForm.status === CourseStatus.ACTIVE
