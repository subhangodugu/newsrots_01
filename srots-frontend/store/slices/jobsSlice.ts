...
export const fetchJobs = (collegeId?: string) => async (dispatch: any) => {
  ...
  try {
    const jobs = await JobService.searchJobs(collegeId || '', {});
...
export const createNewJob = (jobData: any, jdFiles: File[], avoidList: File | null, collegeCode: string) => async (dispatch: any) => {
  dispatch(setLoading(true));
  try {
    const newJob = await JobService.createJob(jobData, jdFiles, avoidList, collegeCode);
